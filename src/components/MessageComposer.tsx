...
const MessageComposer: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const fetchDistinct = async () => {
      try {
        ...
        const cls = Array.from(new Set((classRows || []).map(r => r.class_year).filter((v): v is string => !!v)));
        ...
        const regs = Array.from(new Set((regionRows || []).map(r => r.region).filter((v): v is string => !!v)));
  ...
  // Resend to failed recipients for a selected message
  const loadFailedRecipientsForMessage = async (messageId: string) => {
    if (!messageId) {
      toast({ title: 'Select Message', description: 'Please select a message to load failed recipients', variant: 'destructive' });
      return;
    }
    try {
      setIsProcessing(true);
      setProcessingPhase('Loading failed recipients...');
      let q = supabase
        .from('sms_logs')
      ...
      toast({ title: 'Load Failed', description: 'Could not load failed recipients', variant: 'destructive' });
    } finally {
